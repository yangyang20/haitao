        ...
        <table class="importTpl-table fill_table">
            ...
            <tr>
                ...
                <td> <div class="layui-input-inline">
                        <input type="text" name="name" required value="{{$info->name}}" lay-verify="required" placeholder="请输入标题" autocomplete="off" class="layui-input w200">
                    </div>
                </td>
                ...
                <td> <div class="layui-input-inline">
                        <select name="dealer_id">
                            ...
                            @foreach($dealerList as $item)
                                <option value="{{$item->id}}" {{$info->dealer_id == $item->id ? 'selected' :''}}>{{$item->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </td>
            ...
            <tr>
                ...
                <td> <div class="layui-input-inline">
                        <input type="text" name="start_line" required value="{{$info->start_line}}" lay-verify="required" placeholder="请输入开始行数" autocomplete="off" class="layui-input">
                    </div>
                </td>
            </tr>
            <tr class="config-box">
                ...
                <td colspan="3">
                    <table class="config-table">
                        ...
                        <tr class="item" data-index="{{$config_key}}">
                            ...
                            <td>
                                <div class="layui-input-inline">
                                    <input type="text" class="layui-input" name="table[{{$config_key}}][title]"  value="{{$config_item['title']}}" placeholder="请输入标题名" >
                                </div>
                            </td>
                            <td>
                                <div class="layui-input-inline">
                                ...
                                <div class="layui-input-inline">
                                    <select  name="table[{{$config_key}}][type]">
                                        @foreach($field_type_arr as $key=>$value)
                                            <option value="{{$key}}" {{$config_item['type'] == $key ? 'selected' :''}}>{{$value}}</option>
                                        @endforeach
                                    </select>
                                </div>
{{--                                <div class="layui-input-inline">--}}
